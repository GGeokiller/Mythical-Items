import { EmptyLootItem, Entity, ItemEnchantableComponent, MolangVariableMap, system, world } from "@minecraft/server";
...
EntitySpawnListener.register((event) => {
    ...
    let enchantable = itemStack.getComponent("enchantable") as ItemEnchantableComponent | undefined;
...
system.runInterval(() => {
    ...
    if (tickCounter % particleDensity !== 0) return;
...
function executeItemRarity(item: Entity, rarity: String) {
    let velocity = item.getVelocity();
    if (velocity.y !== 0 && !item.isInWater) return;
    spawnBlighParticle(item, rarity);
}

function spawnBlighParticle(itemEntity: Entity, rarity: String) {
    let molangConfig = new MolangVariableMap();

    let red = ItemRarityColors[rarity as keyof typeof ItemRarityColors].r / 255;
    let green = ItemRarityColors[rarity as keyof typeof ItemRarityColors].g / 255;
    let blue = ItemRarityColors[rarity as keyof typeof ItemRarityColors].b / 255;
